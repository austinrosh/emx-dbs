...
def _resolve_path(path: Optional[Path], config_dir: Path, *, prefer_existing_cwd: bool = True) -> Optional[Path]:
...
def load_config(path: Union[str, Path]) -> OptimizationConfig:
...
def prepare_run_dir(cfg: OptimizationConfig, config_path: Optional[Union[str, Path]] = None) -> Path:
